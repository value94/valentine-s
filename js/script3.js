var Qixi = function () {
    ...
    var girl = {
        elem: $(".girl"), getHeight: function () {
            ...
            $("#talk").css({ left: visualWidth / 2 - 130, top: bridgeY - this.getHeight() - 50 })
        }, getOffset: function () {
    ...
    function BoyToTalk(callback) {
        ...
        function oneByOne() {
            var screen = paper[paper_count].substr(0, count);
            loopText.innerHTML = screen;
            count++;
            if (count > paper[paper_count].length) {
                count = 1;
                paper_count++;
                if(paper_count == paper.length) {
                    callback();
                    return true;
                }
            ...
            setTimeout(oneByOne, 50);
        }
        oneByOne()
        confi.setTime.waitRotate = (paper.length + 5) * 50
    }
